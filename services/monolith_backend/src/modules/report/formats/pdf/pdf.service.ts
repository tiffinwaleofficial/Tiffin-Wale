...
export class PdfService implements IFormatService, OnModuleInit {
  ...
  async onModuleInit() {
    ...
    const configPath = path.join(
      process.cwd(),
      "src/modules/report/formats/pdf/pdf.config.json",
    );
    ...
    const logoPathFromConfig =
      this.config.logo?.path || "src/modules/report/resources/images/logo.jpg";
    ...
    const iconPathFromConfig =
      this.config.icon?.path || "src/modules/report/resources/images/icon.png";
    ...
    const iconMimeType =
      iconExtension === "png"
        ? "image/png"
        : iconExtension === "jpg" || iconExtension === "jpeg"
          ? "image/jpeg"
          : "image/png"; // default fallback
    ...
    if (
      this.config.watermark?.enabled &&
      this.config.watermark?.type === "image"
    ) {
      const watermarkPathFromConfig =
        this.config.watermark.imagePath ||
        "src/modules/report/resources/images/logo_black.png";
      const watermarkPath = path.join(process.cwd(), watermarkPathFromConfig);
      try {
        const watermarkBuffer = await fs.readFile(watermarkPath);

        // Determine MIME type based on file extension
        const watermarkExtension = watermarkPathFromConfig
          .toLowerCase()
          .split(".")
          .pop();
        const watermarkMimeType =
          watermarkExtension === "png"
            ? "image/png"
            : watermarkExtension === "jpg" || watermarkExtension === "jpeg"
              ? "image/jpeg"
              : "image/png"; // default fallback
        this.watermarkImageBase64 = watermarkBuffer.toString("base64");
        this.watermarkMimeType = watermarkMimeType;
        console.log(
          `✅ Watermark image loaded successfully: ${watermarkPathFromConfig} (${watermarkBuffer.length} bytes)`,
        );
      } catch (error) {
        console.warn(
          `⚠️  Warning: Could not load watermark image from ${watermarkPathFromConfig}. Falling back to text watermark. Error: ${error.message}`,
        );
    ...
    const fontsDir = path.join(
      process.cwd(),
      "src/modules/report/resources/fonts",
    );
    const regularFontBuffer = await fs.readFile(
      path.join(fontsDir, "Poppins-Regular.ttf"),
    );
    const boldFontBuffer = await fs.readFile(
      path.join(fontsDir, "Poppins-Bold.ttf"),
    );
    const semiboldFontBuffer = await fs.readFile(
    ...
    const headerPath = path.join(
      process.cwd(),
      "src/modules/report/templates/header.hbs",
    );
    const footerPath = path.join(
      process.cwd(),
      "src/modules/report/templates/footer.hbs",
    );
    ...
    try {
      ...
      console.warn(
        "Warning: Could not load header/footer partials:",
        error.message,
      );
    }
  }
  ...
  private async generatePdfFromTemplate(
    ...
    try {
      ...
      const templatePath = path.join(
        process.cwd(),
        `src/modules/report/templates/${templateName}.hbs`,
      );
      ...
      const cssPath = path.join(
        process.cwd(),
        "src/modules/report/assets/style.css",
      );
      ...
      if (this.config.watermark.enabled) {
        ...
        const useImage =
          this.config.watermark.type === "image" &&
          this.watermarkImageBase64 &&
          this.watermarkMimeType;
      ...
      const logoUrl =
        this.config.header.logoUrl || "https://www.tiffin-wale.com/";

      // Get page margins for header/footer padding
      const pageMargins = this.config.pageMargins || {
        top: 0,
        bottom: 0,
        left: 0,
        right: 0,
      ...
      const pageMarginLeftPx =
        pageMargins.unit === "in" ? pageMargins.left * 96 : pageMargins.left;
      const pageMarginRightPx =
        pageMargins.unit === "in" ? pageMargins.right * 96 : pageMargins.right;
      ...
      const tagline =
        this.config.header.tagline || "Ghar Jaisa Khana Har Jagah";
      ...
      const footerWatermark = this.config.footer.watermark.enabled
        ? `<div style="margin: 4px 0; font-size: ${watermarkFontSize}px; color: ${this.config.footer.watermark.color}; font-style: ${this.config.footer.watermark.style};">${this.config.footer.watermark.text}</div>`
        : "";
      ...
      const pageMarginsForCalc = this.config.pageMargins || {
        top: 0,
        bottom: 0,
        left: 0,
        right: 0,
      ...
      const pageMarginTopIn =
        pageMarginsForCalc.unit === "in"
          ? pageMarginsForCalc.top
          : pageMarginsForCalc.top / 96;
      const pageMarginBottomIn =
        pageMarginsForCalc.unit === "in"
          ? pageMarginsForCalc.bottom
          : pageMarginsForCalc.bottom / 96;
      const pageMarginLeftIn =
        pageMarginsForCalc.unit === "in"
          ? pageMarginsForCalc.left
          : pageMarginsForCalc.left / 96;
      const pageMarginRightIn =
        pageMarginsForCalc.unit === "in"
          ? pageMarginsForCalc.right
          : pageMarginsForCalc.right / 96;
      ...
      const pdfBuffer = await page.pdf({
        format: this.config.page.format as
          | "A4"
          | "Letter"
          | "Legal"
          | "Tabloid"
          ...
          | "A6",
        displayHeaderFooter:
          this.config.header.enabled || this.config.footer.enabled,
