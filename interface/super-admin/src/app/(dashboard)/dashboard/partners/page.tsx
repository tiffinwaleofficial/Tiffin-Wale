...
export default function ManagePartnersPage() {
  ...
  const { data, isLoading: loading, error } = useQuery(
    superAdminControllerGetAllPartnersOptions({ query: { page: 1, limit: 100 } })
  );
  ...
  const updatePartnerStatus = useMutation({
    ...
    onError: (err: any) => {
      toast({
        variant: "destructive",
        title: "Update Failed",
        description: err.message || "Could not update partner status.",
      });
    },
  });
  ...
  const cities = useMemo(() => {
    const citySet = new Set(partners.map((p: any) => p.city));
    return ['all', ...Array.from(citySet)] as string[];
  }, [partners]);

  const handleUpdateStatus = (partnerId: string, newStatus: Partner['status']) => {
